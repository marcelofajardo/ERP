<div template-manager>
    @if(userCan('templates-manage'))
        <p class="text-muted small mb-none">
            {{ trans('bookstack::entities.templates_explain_set_as_template') }}
        </p>
        @include('bookstack::components.toggle-switch', [
               'name' => 'template',
               'value' => old('template', $page->template ? 'true' : 'false') === 'true',
               'label' => trans('bookstack::entities.templates_set_as_template')
        ])
        <hr>
    @endif
    ...
    <div template-manager-list>
        @include('bookstack::pages.template-manager-list', ['templates' => $templates])
    </div>
</div>