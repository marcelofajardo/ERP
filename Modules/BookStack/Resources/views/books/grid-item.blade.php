<a href="{{$book->getUrl()}}" class="grid-card"  data-entity-type="book" data-entity-id="{{$book->id}}">
    ...
    <div class="grid-card-footer text-muted ">
        <p>@icon('star')<span title="{{$book->created_at->toDayDateTimeString()}}">{{ trans('bookstack::entities.meta_created', ['timeLength' => $book->created_at->diffForHumans()]) }}</span></p>
        <p>@icon('edit')<span title="{{ $book->updated_at->toDayDateTimeString() }}">{{ trans('bookstack::entities.meta_updated', ['timeLength' => $book->updated_at->diffForHumans()]) }}</span></p>
    </div>
</a>