...
@section('body')
    ...
    <div class="container" id="search-system">
        ...
        <div class="grid right-focus reverse-collapse gap-xl">
            <div>
                <div>
                    <h5>{{ trans('bookstack::entities.search_filters') }}</h5>

                    <form v-on:submit="updateSearch" v-cloak class="v-cloak anim fadeIn">
                        <h6 class="text-muted">{{ trans('bookstack::entities.search_content_type') }}</h6>
                        <div class="form-group">
                            <label class="inline checkbox text-page"><input type="checkbox" v-on:change="typeChange" v-model="search.type.page" value="page">{{ trans('bookstack::entities.page') }}</label>
                            <label class="inline checkbox text-chapter"><input type="checkbox" v-on:change="typeChange" v-model="search.type.chapter" value="chapter">{{ trans('bookstack::entities.chapter') }}</label>
                            <br>
                            <label class="inline checkbox text-book"><input type="checkbox" v-on:change="typeChange" v-model="search.type.book" value="book">{{ trans('bookstack::entities.book') }}</label>
                            <label class="inline checkbox text-bookshelf"><input type="checkbox" v-on:change="typeChange" v-model="search.type.bookshelf" value="bookshelf">{{ trans('bookstack::entities.shelf') }}</label>
                        </div>

                        <h6 class="text-muted">{{ trans('bookstack::entities.search_exact_matches') }}</h6>
                            ...
                            <tr>
                                <td colspan="2">
                                    <button type="button" class="text-button" v-on:click="addExact">
                                        @icon('add-circle'){{ trans('bookstack::common.add') }}
                                    </button>
                                </td>
                            </tr>
                        </table>

                        <h6 class="text-muted">{{ trans('bookstack::entities.search_tags') }}</h6>
                        <table cellpadding="0" cellspacing="0" border="0" class="no-style">
                        ...
                        <table cellpadding="0" cellspacing="0" border="0" class="no-style form-table">
                            ...
                            <tr>
                                <td>{{ trans('bookstack::entities.search_updated_before') }}</td>
                                <td>
                                    <button type="button" class="text-button" v-if="!search.dates.updated_before"
                                            v-on:click="enableDate('updated_before')">{{ trans('bookstack::entities.search_set_date') }}</button>
                            ...
                            <tr>
                                <td>{{ trans('bookstack::entities.search_created_after') }}</td>
                                <td>
                                    <button type="button" class="text-button" v-if="!search.dates.created_after"
                                            v-on:click="enableDate('created_after')">{{ trans('bookstack::entities.search_set_date') }}</button>
                            ...
                            <tr>
                                <td>{{ trans('bookstack::entities.search_created_before') }}</td>
                                <td>
                                    <button type="button" class="text-button" v-if="!search.dates.created_before"
                                            v-on:click="enableDate('created_before')">{{ trans('bookstack::entities.search_set_date') }}</button>
                        ...
                        <button type="submit" class="button">{{ trans('bookstack::entities.search_update') }}</button>
                    </form>
            ...
            <div>
                <div v-pre class="card content-wrap">
                    <h1 class="list-heading">{{ trans('bookstack::entities.search_results') }}</h1>
                    ...
                    <h6 class="text-muted">{{ trans_choice('bookstack::entities.search_total_results_found', $totalResults, ['count' => $totalResults]) }}</h6>
                    <div class="book-contents">
                        @include('bookstack::partials.entity-list', ['entities' => $entities, 'showPath' => true])
                    </div>
                    @if($hasNextPage)
                        <div class="text-right mt-m">
                            <a href="{{ $nextPageLink }}" class="button outline">{{ trans('bookstack::entities.search_more') }}</a>
                        </div>
                    @endif
                </div>
            </div>
