    ...
    <main class="card content-wrap">
        ...
        <div class="book-content">
            ...
            @else
                <div class="mt-xl">
                    <hr>
                    <p class="text-muted italic mt-xl mb-m">{{ trans('bookstack::entities.shelves_empty_contents') }}</p>
                    <div class="icon-list inline block">
                        ...
                        @if(userCan('bookshelf-update', $shelf))
                            <a href="{{ $shelf->getUrl('/edit') }}" class="icon-list-item text-bookshelf">
                                ...
                                <span>{{ trans('bookstack::entities.shelves_edit_and_assign') }}</span>
                            </a>
                        @endif
                    </div>
                </div>
...
@section('left')
    ...
    @if($shelf->tags->count() > 0)
        <div id="tags" class="mb-xl">
            @include('bookstack::components.tag-list', ['entity' => $shelf])
        </div>
    @endif
    ...
    <div id="details" class="mb-xl">
        ...
        <div class="text-small text-muted blended-links">
            @include('bookstack::partials.entity-meta', ['entity' => $shelf])
            @if($shelf->restricted)
                <div class="active-restriction">
                    @if(userCan('restrictions-manage', $shelf))
                        <a href="{{ $shelf->getUrl('/permissions') }}">@icon('lock'){{ trans('bookstack::entities.shelves_permissions_active') }}</a>
                    @else
                        @icon('lock'){{ trans('bookstack::entities.shelves_permissions_active') }}
                    @endif
                </div>
            @endif
        </div>
        ...
        <div class="icon-list text-primary">
            ...
            @if(userCan('book-create-all') && userCan('bookshelf-update', $shelf))
                <a href="{{ $shelf->getUrl('/create-book') }}" class="icon-list-item">
                    <span class="icon">@icon('add')</span>
                    <span>{{ trans('bookstack::entities.books_new_action') }}</span>
                </a>
            @endif
