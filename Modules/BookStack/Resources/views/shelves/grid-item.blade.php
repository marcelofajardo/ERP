<a href="{{$shelf->getUrl()}}" class="bookshelf-grid-item grid-card"
   data-entity-type="bookshelf" data-entity-id="{{$shelf->id}}">
    ...
    <div class="grid-card-footer text-muted text-small">
        @icon('star')<span title="{{$shelf->created_at->toDayDateTimeString()}}">{{ trans('bookstack::entities.meta_created', ['timeLength' => $shelf->created_at->diffForHumans()]) }}</span>
        <br>
        @icon('edit')<span title="{{ $shelf->updated_at->toDayDateTimeString() }}">{{ trans('bookstack::entities.meta_updated', ['timeLength' => $shelf->updated_at->diffForHumans()]) }}</span>
    </div>
</a>