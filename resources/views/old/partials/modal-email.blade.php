<div id="emailSendModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <form action="{{ route('old.email.send') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="old_id" value="{{ isset($old) ? $old->serial_no : '' }}">
        ...
        <div class="modal-body">
          <div class="form-group">
            <select class="form-control input-sm select-multiple" name="email[]" multiple required>
              ...
              @if ($old->email != '')
                <option value="{{ $old->email }}">{{ $old->email }} - Old Vendor's Email</option>
              @endif

              @if ($old->agents)
                @foreach ($old->agents as $agent)
