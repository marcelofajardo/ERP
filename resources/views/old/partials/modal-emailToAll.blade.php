    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Send Email to Multiple Old</h4>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>

            <form action="{{ route('old.email.send.bulk') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="modal-body">
                    <div class="form-group">
                        <strong>Olds</strong>
                        <select class="form-control select-multiple" name="olds[]" multiple>
                            @foreach ($olds as $old)
                                <option value="{{ $old->serial_no }}">{{ $old->name }} - {{ $old->email }}</option>
                            @endforeach
                        </select>
                    </div>
